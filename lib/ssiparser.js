...
function sync (text, options, clearCache) {
	var i, len, list, index, file, filepath,
        ...
        if (filepath.startsWith('/')) {
            var base = filepath.split('/')[1];
            var idx = -1;
            if (base && (idx = fileRoot.lastIndexOf(base))) {
                fileRoot = fileRoot.substring(0, (idx < 0) ? fileRoot.length : idx);
            }
        }else{
            fileRoot = options.relative;
        }

        filepath = path.join(fileRoot, filepath);
